...
/* 
* il blocco 5 è un multiserver clasico. Bisogna solamente fare la lettura da file
* per gli input e quella di output.
*/
...
#define SERVERS_FIVE 4    /* number of servers              */
...
void *block5() {
    ...
    while ((stopFlag != 4) || (number > 0) || (arrivalsBlockFive != NULL)) {
        ...
        if (e == 0) {                                  /* process an arrival*/
            ...
            if (arrivalsBlockFive == NULL) { /* ArrivalBlockThree queue empty */
                event[0].t = INFINITY;
                event[0].x = 1;
            }
            else {
            ...
            { /* se nel sistema ci sono al più tanti job quanti i server allora calcola un tempo di servizio */
                double service = GetServiceBlockFive();
                printf("\tService: %6.2f\n", service);
    ...
    /* attendi che l'orchestrator ti dia il via libera */
